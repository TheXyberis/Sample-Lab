...
@section('content')
    <div class="container mt-4">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form method="POST"
                    action="{{ isset($sample) ? route('samples.update', $sample->id) : route('samples.store') }}">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                            value="{{ old('name', $sample->name ?? '') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="type" class="form-control @error('type') is-invalid @enderror"
                            value="{{ old('type', $sample->type ?? '') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="quantity" class="form-control"
                            value="{{ old('quantity', $sample->quantity ?? '') }}">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="collected_at" class="form-control"
                            value="{{ old('collected_at', isset($sample) ? $sample->collected_at?->format('Y-m-d') : '') }}">
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Metadata JSON</label>
                        <textarea name="metadata_json" class="form-control"
                            rows="4">{{ old('metadata_json', isset($sample) ? json_encode($sample->metadata_json, JSON_PRETTY_PRINT) : '') }}</textarea>
                    </div>
