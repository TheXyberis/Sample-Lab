...
<div class="container mt-4"> 
    <h3 class="text-primary">{{ isset($measurement) ? 'Edit' : 'Create' }} Measurement</h3> 
    ...
    <form method="POST" action="{{ isset($measurement) ? route('measurements.update', $measurement->id) : route('measurements.store') }}"> 
        ...
        <div class="mb-3"> 
            ...
            <select name="sample_id" class="form-select" required> 
        ...
        <button type="submit" class="btn btn-primary w-100">{{ isset($measurement) ? 'Update' : 'Save' }}</button> 
    </form> 
</div> 
@endsection