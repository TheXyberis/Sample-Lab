                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                @endphp
                                <small class="text-muted">Role: <span
                                        class="badge bg-{{ $roleBadge }}">{{ Auth::user()->role }}</span></small>
                            </div>
                            <div class="text-end">
                ...
                <div class="card shadow-sm border-primary h-100">
                    <div class="card-body text-center">
                        ...
                        <h2 class="display-6 text-primary mb-0">
                            {{ \App\Models\Sample::where('status', 'IN_PROGRESS')->count() }}</h2>
            ...
            <div class="col-12 col-md-3">
                <div class="card shadow-sm border-warning h-100">
                    <div class="card-body text-center">
                        ...
                        <h2 class="display-6 text-warning mb-0">
                            {{ \App\Models\ResultSet::where('status', 'SUBMITTED')->count() }}</h2>
                ...
                <div class="card shadow-sm border-success h-100">
                    <div class="card-body text-center">
                        ...
                        <h2 class="display-6 text-success mb-0">
                            {{ \App\Models\Sample::where('status', 'COMPLETED')->whereDate('updated_at', today())->count() }}
                        </h2>
                ...
                <div class="card shadow-sm border-danger h-100">
                    <div class="card-body text-center">
                        ...
                        <h2 class="display-6 text-danger mb-0">
                            {{ \App\Models\Measurement::where('planned_at', '<', now())->where('status', '!=', 'DONE')->count() }}
                        </h2>
                    ...
                    <div class="card-body">
                        ...
                        @if($recentSamples->count() > 0)
                                    ...
                                    <tbody>
                                        @foreach($recentSamples as $sample)
                                            <tr>
                                                ...
                                                <td><span class="badge bg-{{ $sample->status_badge }}">{{ $sample->status }}</span>
                                                </td>
                                                <td><small class="text-muted">{{ $sample->created_at->diffForHumans() }}</small>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            <div class="text-end mt-3">
                                <a href="{{ route('samples.index') }}" class="btn btn-sm btn-outline-primary">View All
                                    Samples</a>
                            </div>
                        @else
                            <div class="text-center py-4">
                                ...
                                @if(Auth::user()->hasRole(['Admin', 'Manager', 'Laborant']))
                                    <a href="{{ route('samples.create-wizard') }}" class="btn btn-primary btn-sm mt-2">Register
                                        First Sample</a>
                                @endif
                            </div>
                        @endif
