...
<div class="container mt-4"> 
    <h1 class="text-primary">{{ isset($method) ? 'Edit Method' : 'Create Method' }}</h1> 
    ...
    <form method="POST" action="{{ isset($method) ? route('methods.update', $method->id) : route('methods.store') }}"> 
        ...
        <div class="mb-3"> 
            ...
            <textarea class="form-control" name="schema_json" id="schema_json" rows="10" required>{{ isset($method) ? json_encode($method->schema_json, JSON_PRETTY_PRINT) : '{}' }}</textarea> 
        ...
        <div class="mb-3"> 
            ...
            <textarea class="form-control" name="limits_json" id="limits_json" rows="5">{{ isset($method) ? json_encode($method->limits_json, JSON_PRETTY_PRINT) : '{}' }}</textarea> 
        </div> 
 
        <button type="submit" class="btn btn-primary w-100 mb-2">Save Draft</button> 
 
        @if(isset($method)) 
            <button type="button" class="btn btn-warning me-2" id="btn-version">Create New Version</button> 
